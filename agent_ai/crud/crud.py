...
def create_user(db:Session, whatsapp: str):
    db_user = models.User(internal_id=str(uuid.uuid5(NAMESPACE, whatsapp)), whatsapp=whatsapp)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def create_message(db:Session, internal_user_id: str, text: str):
    db_message = models.Message(internal_id=str(uuid.uuid4()), internal_user_id=internal_user_id,text=text)
    db.add(db_message)
    db.commit()
